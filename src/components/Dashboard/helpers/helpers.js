...
export const createCollection = async (collection) => {
  const res = await axios
    .post(collectionApiUrl, collection)
    .then((response) => {
...
export const patchCollection = async (collectionId, updatedData) => {
  const res = await axios
    .patch(`${collectionApiUrl}/${collectionId}`, updatedData)
    .then((response) => {
      console.log("Response:", response.data);
    })
    .catch((error) => {
...
export const calculateBalance = (invoice, collections) => {
  ...
  const outstandingBalance =
    parseFloat(invoice.amount) - totalCollectionsAmount;
...
export const calculateSchoolBalance = (schoolName, invoices, collections) => {
  const schoolInvoices = invoices.filter(
    (invoice) => invoice.client === schoolName
  );
  const schoolCollections = collections.filter(
    (collection) => collection.client === schoolName
  );

  const totalInvoicesAmount = schoolInvoices.reduce(
    (total, invoice) => total + parseFloat(invoice.amount),
    0
  );
  const totalCollectionsAmount = schoolCollections.reduce(
    (total, collection) => total + parseFloat(collection.amount),
    0
  );
