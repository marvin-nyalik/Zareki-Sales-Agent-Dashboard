...
export const updateCollection = createAsyncThunk('collections/updateCollection', async ({ id, updatedData }) => { 
  const response = await axios.patch(`${collectionApiUrl}/${id}`, updatedData);
...
const collectionsSlice = createSlice({
  ...
  reducers: {
    updateInvoiceAmountPaid(state, action) {
      ...
      const invoiceToUpdate = state.invoices.find((invoice) => invoice.invoiceNumber === invoiceNumber);
      if (invoiceToUpdate) {
        const collectionsTotal = state.collections.reduce((total, collection) => {
