...
export const addInvoice = createAsyncThunk('invoices/createInvoice', async (newInvoice) => {
...
export const deleteInvoice = createAsyncThunk('invoices/deleteInvoice', async (invoiceId, { dispatch, getState }) => {
  const state = getState();
  const invoiceToDelete = state.invoices.invoices.find((invoice) => invoice.id === invoiceId);
  if (!invoiceToDelete) {
    throw new Error('Invoice not found');
  }

  await dispatch(deleteCollectionByInvoiceNumber(invoiceToDelete.invoiceNumber));
